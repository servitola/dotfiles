...
Existing public `<topic>.cron` files may already be in the public `cron_jobs/` (e.g. `rag-improve.cron`, `litellm-smoke.cron` — infra-only jobs). When appending to an existing topic, keep using whichever file already exists (public or private) — don't migrate it. New topics always start as `.private.cron` in the overlay.
