...
name: llama-cpp
description: |
  Run local GGUF models with llama.cpp (CPU/Apple Silicon/CUDA/ROCm), pick the right quant, and discover GGUF repos on the Hugging Face Hub.

  Use when: "запусти модель локально через llama.cpp", "подбери квант GGUF", "найди GGUF на huggingface", "run a local GGUF model", "which quant should I use", "serve a model with llama-server"
---
...
```bash
# Install llama.cpp (CLI + server: llama-cli, llama-server)
brew install llama.cpp            # macOS / Linux
# winget install llama.cpp        # Windows
# or build from source: https://github.com/ggml-org/llama.cpp

# Python bindings (optional, for the llama-cpp-python examples below)
