...
## Debugging an Ink + tsx TUI

For a TUI built on Ink + tsx, two common scenarios:
...
If the misbehaving subprocess is Python rather than Node, use the `python-debugpy` skill for it. Only the Node portions (Ink UI, client code, tsx-run tests) use this skill.
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. Launch it in a background/interactive shell that lets you send subsequent commands to the same process. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
