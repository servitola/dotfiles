...
For the `curl` fallback, export a personal access token (with `repo` scope) as `GITHUB_TOKEN`. Setting repo secrets via `curl` also needs Python with PyNaCl (`pip install pynacl`).
