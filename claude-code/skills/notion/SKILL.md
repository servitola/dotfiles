...
name: notion
description: |
  Talk to Notion via the official ntn CLI or HTTP/curl: pages, databases, markdown, file uploads, Workers.

  Use when: "создай страницу в notion", "запиши в notion", "найди в ноушене", "create a notion page", "query notion database", "upload to notion"
---
...
3. Store it as the `NOTION_API_KEY` environment variable (e.g. export it from your shell profile or a secrets file like `~/.config/openai_key.sh`):
   ```
...
Add those exports to your shell profile so every session inherits them.
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via your harness's MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
