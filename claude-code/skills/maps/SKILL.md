...
name: maps
description: |
  Geocode places, find nearby POIs, compute routes/distances, and look up timezones via free OpenStreetMap/OSRM data (no API key).

  Use when: "найди ближайшие кафе", "geocode this address", "сколько ехать от X до Y", "построй маршрут", "find restaurants near", "what timezone is", "directions from A to B"
---
...
Script path: `~/projects/dotfiles/claude-code/skills/maps/scripts/maps_client.py`

## Commands

```bash
MAPS=~/projects/dotfiles/claude-code/skills/maps/scripts/maps_client.py
```
...
with the WebSearch tool since OSM hours are community-maintained and not
always current.
...
```bash
python3 ~/projects/dotfiles/claude-code/skills/maps/scripts/maps_client.py search "Statue of Liberty"
# Should return lat ~40.689, lon ~-74.044

python3 ~/projects/dotfiles/claude-code/skills/maps/scripts/maps_client.py nearby --near "Times Square" --category restaurant --limit 3
