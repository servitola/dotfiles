...
description: |
  Serve LLMs at high throughput with vLLM: OpenAI-compatible API, PagedAttention, continuous batching, quantization (AWQ/GPTQ/FP8), and tensor parallelism.

  Use when: "запусти vllm сервер", "разверни llm на gpu", "ускорь инференс llm", "serve a model with vllm", "high-throughput llm api", "quantize and serve a 70B model"
---
...
vLLM requires a CUDA-capable GPU (NVIDIA primary; AMD ROCm / Intel GPUs / TPUs also supported) plus `torch` and `transformers`, which `pip install vllm` pulls in.
