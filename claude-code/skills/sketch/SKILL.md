...
name: sketch
description: |
  Explore a UI/UX direction as 2-3 throwaway, side-by-side HTML mockups so the user can compare design stances before committing to one.

  Use when: "набросай этот экран", "покажи варианты интерфейса", "сравни пару макетов", "sketch this screen", "show me 2-3 takes on this UI", "mockup this before I build"
---
...
**Verify variants visually — use whatever browser/screenshot tools are available (e.g. a Playwright MCP).** Don't just write HTML and hope it renders; load each variant and look at it:

```
browser_navigate(url="file:///absolute/path/to/sketches/001-calm-editorial/index.html")
browser_snapshot / screenshot → "Does this layout look clean and readable? Any visible bugs (overlapping text, unstyled elements, broken images)?"
```

A page snapshot/screenshot catches layout bugs that pure source inspection misses (e.g. a font import that silently failed, a flex container that collapsed). Fix and re-navigate until each variant looks right. If no browser tool is available, at least `open` the file and ask the user to confirm.
...
```
Bash: mkdir -p sketches/001-calm-editorial
Write sketches/001-calm-editorial/index.html  → "<!doctype html>..."
Write sketches/001-calm-editorial/README.md   → "## Variant: Calm editorial\n..."
browser_navigate(url="file://$(pwd)/sketches/001-calm-editorial/index.html")
screenshot / snapshot → "How does this look? Any obvious layout issues?"
```
