...
def fetch_all(owner: str, repo: str, number: int) -> dict[str, Any]:
    ...
    # Per-connection exhausted state. Without it, a connection that finishes
    # paginating before the others would have its cursor reset to None and its
    # first page re-fetched on subsequent loops, duplicating results.
    comments_done = False
    reviews_done = False
    threads_done = False
    ...
    while True:
        ...
        # Only collect from connections that are still paginating; exhausted
        # connections get re-fetched by the shared query but must be ignored.
