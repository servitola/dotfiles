...
Basic sketches need only a browser — the core library loads from a CDN, no install required. The optional export pipeline needs:

- **Node.js + npm** — for headless rendering: `brew install node`
- **Puppeteer** — headless high-res frame capture: `npm install puppeteer`
- **ffmpeg** — encode frame sequences into MP4: `brew install ffmpeg`
- **Python 3** — local server for loading fonts/images: ships with macOS (`python3 -m http.server`)

Run `bash scripts/setup.sh` to verify which tools are present.
...
8. **Load references on demand** — use `Read` on the `references/...` files to load specific reference material as needed during implementation
