...
The CLI is the agent's command layer. It turns a service, app, API, log source, or database into shell commands the agent can run repeatedly from any repo.

Good agent CLIs expose composable primitives. Smaller discover, read, resolve, download, inspect, draft, and upload commands compose better than a single command that tries to "do the whole investigation".

## Help is interface

Write `--help` for a future agent thread that only has the binary and a vague task. Each command gets a short description and flags with literal names from the product or API.
...
tool-name --json channels resolve --name claude
...
The important rule is consistency. Pick one style and stay with it unless the product vocabulary demands otherwise.
...
Once the agent has a stable ID, every later command should accept it directly — repeated broad searches waste tokens and time.
...
For exit codes (fail-fast — surface problems loudly):
...
- Make `doctor --json` usable even when auth is missing. It reports missing auth rather than crashing.
...
Treat raw writes as live writes. Expose POST/PUT/PATCH/DELETE explicitly rather than hiding them behind a "debug" command.

## Companion skill pattern

The companion skill should be smaller than the CLI README. It teaches the path through the tool:
...
- Publish/delete/retry/submit only when the user asked.
