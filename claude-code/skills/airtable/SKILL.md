...
name: airtable
description: |
  Read and write Airtable bases, tables, and records directly via the REST API and curl — CRUD, filterByFormula, upserts, pagination.

  Use when: "найди записи в airtable", "добавь строку в airtable", "обнови запись в airtable", "query my airtable base", "add a record to airtable", "update airtable rows"
---

# Airtable — Bases, Tables & Records

Work with Airtable's REST API directly via `curl` in Bash. No MCP server, no OAuth flow, no Python SDK — just `curl` and a personal access token.
...
4. Store the token as the `AIRTABLE_API_KEY` environment variable (e.g. export it from your shell profile or a secrets file like `~/.config/openai_key.sh`):
   ```
...
- **Always use Bash with `curl`.** Do NOT use a web-fetch tool (it can't send auth headers) or a browser (needs UI auth and is slow).
- **Make sure `AIRTABLE_API_KEY` is exported in the environment** before running `curl` calls (see Prerequisites).
