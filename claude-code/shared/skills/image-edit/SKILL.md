...
Fill Pro, ~$0.05. Cleaner edge integration and sharper region content
...
#### Building non-rectangular masks: `make_mask.py`

For regions that aren't a single rectangle — a ring around an arched
mirror, an ellipse around someone's face, a donut excluding the centre
— compose the mask from primitives. Each `--add SHAPE` paints white;
each `--sub SHAPE` carves black. Order is preserved.

Shapes (all coords in input pixels; suffix `%` accepted):
- `rect:x,y,w,h`
- `ellipse:cx,cy,rx,ry`
- `arch:x,y,w,h` — rectangle with semicircular top, exactly the mirror
  shape friend works with
- `ring:cx,cy,r_out,r_in` — annulus

Recipes for recurring patterns in friend's «Идеи» topic:

```bash
# Repaint the green frame around an arched mirror (the ring between
# outer arch and inner arch). Soft feather for clean blend.
uv run ~/projects/dotfiles/claude-code/shared/skills/image-edit/make_mask.py \
  --like /tmp/prev.jpg --output /tmp/mask.png --feather 6 \
  --add arch:140,60,520,720 \
  --sub arch:200,120,400,600 \
  --preview /tmp/preview.png

# Repaint only the neon caption area on the upper third of the mirror
uv run ~/projects/dotfiles/claude-code/shared/skills/image-edit/make_mask.py \
  --like /tmp/prev.jpg --output /tmp/mask.png --feather 4 \
  --add rect:200,120,400,200

# Thin down the gold frame of the arched mirror (touch only the frame
# itself, not the glass or the wall): subtract a slightly-bigger arch
# from a same-sized arch — leaves a thin border to repaint.
uv run ~/projects/dotfiles/claude-code/shared/skills/image-edit/make_mask.py \
  --like /tmp/prev.jpg --output /tmp/mask.png --feather 3 \
  --add arch:200,120,400,600 \
  --sub arch:210,130,380,580
```

Always pass `--preview` for ring/arch masks and **show the preview to
the user** via `mcp__bot__send_image` before spending an inpaint call,
especially the paid one. Caption it like «вот эту область перекрашу —
ок?». Five seconds of confirmation saves a wasted paid call when the
bbox is off.
