...
name: design-md
description: |
  Author, validate, diff, and export Google's DESIGN.md design-token spec files (with WCAG contrast checks and Tailwind/DTCG export).

  Use when: "сделай DESIGN.md", "опиши дизайн-токены", "проверь контраст палитры", "create a DESIGN.md", "author design tokens", "check WCAG contrast"
---
...
The lint/diff/export CLI runs through Node. No global install needed — `npx`
fetches it on demand:

```bash
# Requires Node.js (provides npx)
node --version || brew install node

# The CLI is invoked via npx (auto-downloads on first run)
...
1. **Ask the user** (or infer) the brand tone, accent color, and typography
   ...
   token shape above.
2. **Write `DESIGN.md`** in their project root using `Write`. Always
